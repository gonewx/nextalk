...
async def main():
    import os
    # Check if SSL certificates exist and are provided
    use_ssl = (len(args.certfile) > 0 and len(args.keyfile) > 0 and 
               os.path.isfile(args.certfile) and os.path.isfile(args.keyfile))
    
    if use_ssl:
        print(f"Starting WebSocket server with SSL on wss://{args.host}:{args.port}")
        ...
        print(f"Starting WebSocket server without SSL on ws://{args.host}:{args.port}")
        async with websockets.serve(
