...
5. **或等待自动提交** - 停顿后自动提交文字
6. **按 Esc 取消** - 录音中按 Esc 放弃本次输入，文字不会上屏（由 Fcitx5 插件或 GNOME 扩展捕获，其余时间 Esc 不受影响）

> **自动纠错**: 上屏前会自动修正全大写英文、口头禅（嗯、呃）、重复词和中文语境下的半角标点。可在 `~/.config/nextalk/settings.yaml` 的 `text:` 节调整开关，并通过 `replacements` 配置自定义纠错词典（如 `"派森": "Python"`）。
