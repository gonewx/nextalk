...
def mock_file_system():
    ...
    with (
        ...
        yield {"exists": mock_exists, "expanduser": mock_expanduser, "makedirs": mock_makedirs}
...
def mock_package_config_path():
    ...
    mock_path.__truediv__ = mock.MagicMock(
        return_value=Path("/mock/package/config/default_config.ini")
    )
...
def test_load_config_with_default_values(
    mock_config_parser, mock_file_system, mock_package_config_path
):
    """测试加载配置时使用内置默认值"""
    # 设置文件不存在，强制使用内置默认值
...
def test_load_config_from_user_file(mock_config_parser, mock_file_system, mock_package_config_path):
    """测试从用户配置文件加载配置"""
    # 设置用户配置文件存在
    mock_file_system["exists"].side_effect = (
        lambda path: path == "/mock/home/.config/nextalk/config.ini"
    )
...
def test_create_default_user_config_from_package(
    mock_file_system, mock_shutil, mock_package_config_path
):
    """测试从包配置文件创建用户配置文件"""
...
def test_load_config_with_custom_path():
    ...
    with mock.patch("os.path.exists", return_value=True):
        with mock.patch("configparser.ConfigParser", autospec=True) as mock_config:
            ...
            mock_instance.get = lambda section, option, fallback=None: config_data[section].get(
                option, fallback
            )

            # 调用被测试的函数
            with mock.patch(
                "nextalk_client.config.loader.ConfigParser", return_value=mock_instance
            ):
