...
*   **保障隐私与高性能**：集成 Sherpa-onnx 离线模型，确保数据不出本地，并实现 <20ms 的上屏延迟。
...
*   **FR6 [系统]: 全局快捷键**
    ...
    *   **实现方式**: 系统原生快捷键 (如 GNOME 设置 → 键盘 → 自定义快捷键) + `nextalk --toggle` 命令。
    *   > **SCP-002 变更**: 原 Fcitx5 插件侧快捷键监听机制已移除，改为更简单的系统原生快捷键方案。
