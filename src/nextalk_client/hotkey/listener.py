...
class HotkeyListener:
    ...
    def start(
        self, hotkey_combination: HotkeyCombination, on_activate: Callable, on_deactivate: Callable
    ) -> bool:
        """
        启动热键监听。
    ...
    def _is_hotkey_matched(self) -> bool:
        ...
        logger.debug(
            f"热键匹配检查: {result} (热键组合: {self._hotkey_combo}, 当前按下的键: {self._pressed_keys})"
        )
        return result
                                ...
                                while retry_count < max_retries and not success:
                                    try:
                                        future = asyncio.run_coroutine_threadsafe(
                                            self._on_deactivate(), client.loop
                                        )
                                        ...
                                        logger.warning(
                                            f"尝试执行热键释放回调失败(尝试 {retry_count + 1}/{max_retries}): {e}"
                                        )
                                        retry_count += 1
                                        # 短暂等待后重试
                                        time.sleep(0.1)
