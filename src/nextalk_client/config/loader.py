...
class Config:
    ...
    def __init__(
        self, config_data: Dict[str, Dict[str, str]] = None, config_path: Optional[str] = None
    ):
        """
        初始化配置访问器。
    ...
    def get_int(self, section: str, key: str, default: int = 0) -> int:
        ...
        try:
            ...
            logger.warning(
                f"无法转换为整数的配置: [{section}].{key}={value}，使用默认值: {default}"
            )
            return default
    ...
    def get_float(self, section: str, key: str, default: float = 0.0) -> float:
        ...
        try:
            ...
            logger.warning(
                f"无法转换为浮点数的配置: [{section}].{key}={value}，使用默认值: {default}"
            )
            return default

    def get_list(
        self, section: str, key: str, default: List[str] = None, delimiter: str = ","
    ) -> List[str]:
        """
        获取列表配置值（以分隔符分隔的字符串）。
        ...
        if not isinstance(value, str):
            logger.warning(
                f"无法转换为列表的配置: [{section}].{key}={value}，使用默认值: {default}"
            )
            return default
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    加载顺序:
    1. 首先尝试从指定路径或默认用户配置文件(~/.config/nextalk/client.ini)加载配置。
    ...
    if config_path is None:
        user_config_path = os.path.expanduser("~/.config/nextalk/client.ini")
        # 支持从旧路径迁移
        old_user_config_path = os.path.expanduser("~/.config/nextalk/config.ini")
        if os.path.exists(old_user_config_path) and not os.path.exists(user_config_path):
            try:
                import shutil

                os.makedirs(os.path.dirname(user_config_path), exist_ok=True)
                shutil.copy2(old_user_config_path, user_config_path)
                logger.info(
                    f"客户端配置已从旧路径迁移: {old_user_config_path} -> {user_config_path}"
                )
            except Exception as e:
                logger.error(f"客户端配置迁移失败: {e}")
    else:
...
def create_default_user_config() -> bool:
    ...
    如果用户配置文件不存在，将默认配置文件复制到~/.config/nextalk/client.ini。
    ...
    if os.path.exists(old_user_config_path) and not os.path.exists(user_config_path):
        try:
            if not ensure_config_directory():
                return False
            shutil.copy2(old_user_config_path, user_config_path)
            logger.info(f"客户端配置已从旧路径迁移: {old_user_config_path} -> {user_config_path}")
        except Exception as e:
            logger.error(f"客户端配置迁移失败: {e}")
            return False
    ...
    if os.path.exists(package_config_path):
        try:
            # 从默认配置文件中只提取[Client]部分
            default_config = configparser.ConfigParser()
            default_config.read(package_config_path)

            # 创建新的客户端配置
            client_config = configparser.ConfigParser()

            # 只复制[Client]部分
            if default_config.has_section("Client"):
                client_config.add_section("Client")
                for key, value in default_config["Client"].items():
                    client_config.set("Client", key, value)

            # 保存客户端配置
            with open(user_config_path, "w", encoding="utf-8") as f:
                client_config.write(f)

            logger.info(f"已创建客户端配置文件: {user_config_path}")
            return True
        except Exception as e:
            logger.error(f"创建客户端配置文件时出错: {str(e)}")
            return False
    else:
        ...
        try:
            ...
            # 使用内置默认值，只创建[Client]部分
            config.add_section("Client")
            config.set("Client", "hotkey", "ctrl+shift+space")
            config.set("Client", "server_url", "ws://127.0.0.1:8010/ws")
            config.set("Client", "locale", "zh")
            config.set("Client", "show_notifications", "false")
            config.set("Client", "enable_injection", "true")
            config.set("Client", "show_text", "false")
            config.set("Client", "audio_backend", "pulse")
