...
class SystemTrayIcon:
                ...
                if icon_path.exists():
                    ...
                    if filename.lower().endswith(".svg"):
                        png_data = cairosvg.svg2png(
                            url=str(icon_path), output_width=actual_size, output_height=actual_size
                        )
                    ...
                    logger.debug(
                        f"已加载图标: {filename}, 尺寸: {self.icons[state].width}x{self.icons[state].height}"
                    )
                else:
    ...
    def start(
        ...
        model_select_callback: Optional[Callable[[str], None]] = None,
    ) -> bool:
        """
        启动系统托盘图标。
        ...
        try:
            ...
            if model_select_callback:
                for model in self.AVAILABLE_MODELS:
                    model_menu_items.append(
                        MenuItem(
                            model,
                            lambda item, model_name=model: self._handle_model_select(
                                model_name, model_select_callback
                            ),
            ...
            if model_select_callback and model_menu_items:
                menu_items.append(MenuItem("选择模型", Menu(*model_menu_items)))

            # 添加退出菜单项
            menu_items.append(MenuItem("退出", lambda: self._handle_quit(on_quit)))
            ...
            self._icon_thread = threading.Thread(
                target=self._run_icon, name="NexTalk-TrayIcon", daemon=True
            )
