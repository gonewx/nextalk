...
bool Tray::init_indicator_api() {
  ...
  do {
    ...
    // Nextalk patch: 优先加载 ayatana 实现。上游硬编码旧版
    // libappindicator3——Debian 系靠 ayatana 包的兼容 symlink 兜底,
    // 而 Fedora 上该名字由已废弃的 libappindicator-12.10.1 提供,其
    // app_indicator_set_status 在 GNOME 49/Wayland 下段错误。
